...
@Component({
  ...
  styleUrls: ['./client-finish.component.css',
...
export class ClientFinishComponent implements OnInit {
  ...
  getDocs() {
    ...
    this._clientService.getDocsByClientId(this.client_id)
      .subscribe(data => {
        ...
        $('#addDocsButton, #deleteDocsButton, #savePDFButton').addClass('disabled');
        this.add_ids = [];
        this.delete_ids = [];
  ...
  checkChanged(flag = '', type = 'completed') {
    ...
    if(type == 'completed') {
      $('#completedAllCheckbox')[0].checked = checked_docs.length == this.completed_docs.length;
    } else {
      $('#notCompletedAllCheckbox')[0].checked = checked_docs.length == this.not_completed_docs.length;
    }
  }
  ...
  checkAllChanged(type = 'completed') {
    if(type == 'completed') {
      ...
      for (let i in this.not_completed_docs) {
        this.not_completed_docs[i].checked = $('#notCompletedAllCheckbox')[0].checked;
      }
    }
  ...
  addDocs() {
    ...
    this._docService.selectDocs(this.client_id, this.add_ids)
      .subscribe(data => {
        this.getDocs();
      });
  }
  ...
  deleteDocs() {
    ...
    if (confirm('Are you sure to delete the selected forms?')) {
      ...
      this._docService.deleteDocs(this.client_id, this.delete_ids)
        .subscribe(data => {
          this.getDocs();
        });
    }
