...
export class FormHelper {
  ...
  static getDomById(id, type = 'normal') {
    ...
    let attributes = style + ' id="' + type + 'cell_' + item.id + '" class="form-cell" ' + item_attributes;
    let dom;
    switch (item.type) {
      case 'root':
        dom = $('<div></div>');
          ...
          if (item.validation_type == 'date') {
            ...
            dom.change(function() {
              ...
              if(!Common.validatePhone($(this).val())) {
                let message = item.validation_label + ' is invalid phone number.<br/>Please type xxx-xxxxxxxx for phone number.';
                console.error(message);
                Notification.notifyError(message);
              }
            });
  ...
  static createBoxes(dom, item, type = 'radio', main_type = 'normal') {
    ...
    for (let i in array) {
      ...
      let radio = $('<label class="' + class_name + '">' + row.label + '</label>');
