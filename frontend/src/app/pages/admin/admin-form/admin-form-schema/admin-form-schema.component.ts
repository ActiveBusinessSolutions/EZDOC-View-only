...
export class AdminFormSchemaComponent implements OnInit {
      ...
      function InitEvents() {
        ...
        $('#cellTypeSelector').on('select', function () {
          ...
          if (FormHelper.isInput(type)) {
            ...
            $('#keyContainer, #structureContainer, #foreignKeyContainer, #wizardButtonContainer, #validationContainer').show();
          } else {
            $('#contentContainer').show();
            $('#keyContainer, #structureContainer, #foreignKeyContainer, #wizardButtonContainer, #wizardToolsContainer, #validationContainer').hide();
          }
  ...
  getForms() {
    ...
    this._formService.getForms()
      .subscribe(data => {
        ...
        $('#formSelector').jqxDropDownList({
          source: dataAdapter, displayMember: "type", valueMember: "id"
        });
  ...
  extractTemplate() {
    ...
    $('#pageSelector').jqxDropDownList('selectIndex', parseInt(pageIndex) - 1);
  ...
  refreshTree() {
    ...
    $('#schemaTree').jqxTree('expandItem', $("#schemaTree").find('li:first')[0]);
  ...
  addItem(type = 'normal') {
    if (this.selectedItem != null) {
      ...
      if (FormHelper.isInput(item.type)) {
        item['key'] = $('#keyInput').val();
        item['foreign_key'] = $('#foreignKeyInput').val();
  ...
  selectItemById(id) {
    ...
    $('#validationTypeSelector').jqxDropDownList('val', item.validation_type || 'none');
